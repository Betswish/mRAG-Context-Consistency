...
do
	...
	do
		sbatch --time=1-00:00:00 --ntasks=4 --cpus-per-task=4 --mem=180G --partition=gpu_h100 --nodes=1 --gpus-per-node=1 --wrap="conda run -n RAGConsis CUDA_VISIBLE_DEVICES=0 python attribute_GMMLU_choice_multi.py --mname $mname --lang $lang" --output=log_attri/GMMLU_choice_multi/$mname/$lang.%j.out
	done
done
