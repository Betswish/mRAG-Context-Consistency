...
do
	...
	do
                sbatch --time=3:00:00 --ntasks=4 --cpus-per-task=4 --mem=180G --partition=gpu_h100 --nodes=1 --gpus-per-node=1 --wrap="conda run -n RAGConsis CUDA_VISIBLE_DEVICES=0 python GMMLU_choice_multi.py --mname $mname --lang $lang" --output=log/GMMLU_choice_multi/$mname/$lang.%j.out --job-name=${mname:0:2}\_$lang            
	done
done
