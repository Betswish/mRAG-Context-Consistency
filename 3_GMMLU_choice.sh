...
do
	...
	do
                sbatch --time=6:00:00 --ntasks=4 --cpus-per-task=4 --mem=180G --partition=gpu_h100 --nodes=1 --gpus-per-node=1 --wrap="conda run -n RAGConsis CUDA_VISIBLE_DEVICES=0 python GMMLU_choice.py --mname $mname --lang $lang" --output=log/GMMLU_choice/$mname/$lang.%j.out              
	done
done
