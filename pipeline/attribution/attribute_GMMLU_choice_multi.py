...
def main():
    ...
    save_dir = f"mirage/GMMLU_choice_multi/{mname}/"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    with open(f"results/GMMLU_choice_multi/{mname}_{args.lang}.json", encoding='utf-8') as f:
        data = json.load(f)
    f.close()
    

    with open("configs/prompts/instruction_choice.json", encoding='utf-8') as f:
        instructions = json.load(f)
    f.close()

    success_cases = []
    success_path = f"mirage/GMMLU_choice_multi/{mname}/success_{args.lang}.json"
    for ins_id, ins in enumerate(tqdm(data)):
        try:
            for postfix in tqdm(["1o3i_0", "1o3o_0", "3o1i_0", "3o1o_0", "1i3i_0", "1i3o_0",  "3i1i_0", "3i1o_0"]):
            # for postfix in tqdm(["1o3i_1", "1o3o_1", "1o3i_2", "1o3o_2", "3o1i_1", "3o1o_1", "3o1i_2", "3o1o_2"]):
            # for postfix in tqdm(["1i3i_1", "1i3o_1", "1i3i_2", "1i3o_2", "3i1i_1", "3i1o_1", "3i1i_2", "3i1o_2"]):
                if "aya" in mname.lower():
                    input_template=f"<BOS_TOKEN><|START_OF_TURN_TOKEN|><|SYSTEM_TOKEN|>{instructions[args.lang]}<|END_OF_TURN_TOKEN|><|START_OF_TURN_TOKEN|><|USER_TOKEN|>"+"{context}\n{current}<|END_OF_TURN_TOKEN|>"
                    input_context_text = '\n'.join(ins[f'prompt_{postfix}'].split('<|USER_TOKEN|>')[1].split('\n')[:4])
                    input_current_text = '\n'.join(ins[f'prompt_{postfix}'].split('<|USER_TOKEN|>')[1].split('\n')[4:]).split('<|END_OF_TURN_TOKEN|>')[0]
                    output_template = "<|START_OF_TURN_TOKEN|><|CHATBOT_TOKEN|>\n{current}"
                    output_current_text = ins[f'response_{postfix}']
                    ...
                    input_template = f"<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\nCutting Knowledge Date: December 2023\nToday Date: 04 Mar 2025\n\n{instructions[args.lang]}<|eot_id|><|start_header_id|>user<|end_header_id|>\n\n"+"{context}\n{current}<|eot_id|>"
                    input_context_text = '\n'.join(ins[f'prompt_{postfix}'].split("<|start_header_id|>user<|end_header_id|>\n\n")[1].split('\n')[:4])
                    input_current_text = '\n'.join(ins[f'prompt_{postfix}'].split("<|start_header_id|>user<|end_header_id|>\n\n")[1].split('\n')[4:]).split('<|eot_id|>')[0]
                    ...
                    input_template = f"<bos><start_of_turn>user\n{instructions[args.lang]} "+"{context}\n{current}<end_of_turn>"
                    input_context_text = '\n'.join(ins[f'prompt_{postfix}'].split(f"{instructions[args.lang]} ")[1].split('\n')[:4])
                    input_current_text = '\n'.join(ins[f'prompt_{postfix}'].split(f"{instructions[args.lang]} ")[1].split('\n')[4:]).split('<end_of_turn>')[0]
                    ...
                    input_template = f"<|im_start|>system\n{instructions[args.lang]}<|im_end|>\n<|im_start|>user\n"+"{context}\n{current}\n<|im_end|>"
                    input_context_text = '\n'.join(ins[f'prompt_{postfix}'].split('<|im_start|>user\n')[1].split('\n')[:4])
                    input_current_text = '\n'.join(ins[f'prompt_{postfix}'].split('<|im_start|>user\n')[1].split('\n')[4:])
                ...
                save_path = save_dir + f"{ins_id}_{args.lang}_{postfix}.json"
            ...
            success_cases.append(ins_id)
            if len(success_cases) >= 50: 
                break
        except:
            continue
