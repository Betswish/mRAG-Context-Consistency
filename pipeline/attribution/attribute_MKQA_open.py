...
def main():
    ...
    save_dir = f"mirage/MKQA_open/{mname}/"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    with open(f"results/MKQA_open/{mname}_{args.lang}.json", encoding='utf-8') as f:
        data = json.load(f)
    f.close()
    ...
    failure_cases = []
    failure_path = f"mirage/MKQA_open/{mname}/failures.json"

    for ins_id, ins in enumerate(tqdm(data)):
        for lang in ["inlang_0", "inlang_1", "inlang_2", "outlang_0", "outlang_1", "outlang_2"]:
