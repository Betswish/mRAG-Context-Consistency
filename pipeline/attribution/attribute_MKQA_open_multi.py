...
def main():
    ...
    save_dir = f"mirage/MKQA_open_multi/{mname}/"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    with open(f"results/MKQA_open_multi/{mname}_{args.lang}.json", encoding='utf-8') as f:
        data = json.load(f)
    f.close()
    

    with open("configs/prompts/instruction_open.json", encoding='utf-8') as f:
        instructions = json.load(f)
    f.close()

    success_cases = []
    success_path = f"mirage/MKQA_open_multi/{mname}/success_{args.lang}.json"
        ...
        try:
            ...
            # for postfix in tqdm(["1i3i_1", "1i3o_1", "1i3i_2", "1i3o_2", "3i1i_1", "3i1o_1", "3i1i_2", "3i1o_2"]):
                if "aya" in mname.lower():
                    ...
                    input_current_text = ins[f'prompt_{postfix}'].split('<|USER_TOKEN|>')[1].split('\n')[-1].split('<|END_OF_TURN_TOKEN|>')[0]
                    ...
                    input_current_text = ins[f'prompt_{postfix}'].split("<|start_header_id|>user<|end_header_id|>\n\n")[1].split('\n')[-1].split('<|eot_id|>')[0]
                    ...
                    input_context_text = '\n'.join(ins[f'prompt_{postfix}'].split(f"{instructions[args.lang]['ctx']} ")[1].split('\n')[:-1])
                    input_current_text = ins[f'prompt_{postfix}'].split(f"{instructions[args.lang]['ctx']} ")[1].split('\n')[-1].split('<end_of_turn>')[0]
                    ...
                    input_current_text = ins[f'prompt_{postfix}'].split('<|im_start|>user\n')[1].split('\n')[-1]
