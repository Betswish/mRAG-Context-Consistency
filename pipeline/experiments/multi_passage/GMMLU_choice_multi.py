...
def main():
    ...
    save_dir = f"results/{task}_choice_multi/"
    ...
    for ins_id, ins in enumerate(tqdm(data)):
        ...
        if gold_docs_inlang_cand and gold_docs_outlang_cand and dist_docs_inlang_cand and dist_docs_outlang_cand:
            ...
            option_segment = "\n"
            for char in ["A", "B", "C", "D"]:
                option_segment += char + ". " + ins[char] + "\n"

            #doc_template = "Document [{ID}](Title: {T}): {P}\n"
            doc_template = "[{ID}]({T}): {P}\n"
            
            meta_info = {"index": ins['index'], "answer": ins['answer']}
            prompts = []

            # (1) non context
            #content_non_ctx = "Question: " + ins['question'] + " Answer:"
            content_non_ctx = f"{ins['question']}\n" + option_segment
            prompt_non_ctx = make_prompt(tokenizer, args.mname, instructions[args.lang], content_non_ctx)
            ...
            for i in range(3):
                ...
                docs_no, gold_docs_outlang_langs, _  = shuffle_docs(gold_docs_outlang)
                docs_in, dist_docs_inlang_langs, _   = shuffle_docs(gold_docs_outlang+dist_docs_inlang)
                docs_out, dist_docs_outlang_langs, _ = shuffle_docs(gold_docs_outlang+dist_docs_outlang)
                ...
                prompt_no  = make_prompt(tokenizer, args.mname, instructions[args.lang], content_no)
                prompt_in  = make_prompt(tokenizer, args.mname, instructions[args.lang], content_in)
                prompt_out = make_prompt(tokenizer, args.mname, instructions[args.lang], content_out)
            ...
            for i in range(3):
                ...
                docs_out, dist_docs_outlang_langs, _ = shuffle_docs(gold_docs_inlang+dist_docs_outlang)
        ...
        if (((count_ins % args.batch_size) == 0) or ((ins_id+1) == num_ins)) and batch_prompts:
            ...
            for raw_response_id, raw_response in enumerate(seg_raw_responses_list):
                ...
                save_data[current_id][field_correct] = contains_option(save_data[current_id]['answer'], raw_response.outputs[0].text)
